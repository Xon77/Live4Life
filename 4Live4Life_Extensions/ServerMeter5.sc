ServerMeterView5 {
	...
	classvar serverMeterViews, updateFreq = 5, dBLow = -80, meterWidth = 12, gapWidth = -1, <height = 230;
	...
	init { arg aserver, parent, leftUp, anumIns, anumOuts;
		...
		if(numOuts > 0) {
			...
			outmeters = Array.fill( numOuts, { arg i;
				...
				StaticText(comp, Rect(0, 83/*180*/, meterWidth -3, 40)).align_(\topLeft)
				.font_(Font.sansSerif(8).boldVariant)
				.string_((i+1).asString)
				.stringColor_( case {i<8} {Color.yellow}  {i.inclusivelyBetween(8,15)} {Color.red}  {i.inclusivelyBetween(16,23)} {Color.blue} {i>23} {Color.yellow} );
				levelIndic = LevelIndicator( comp, Rect(0, 0, meterWidth, 80/*180*/) ).warning_(0.9).critical_(1.0)
				.drawsPeak_(true)
				.numTicks_(9)
				.numMajorTicks_(3)
