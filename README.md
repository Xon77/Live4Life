...
2. Contrary to most tools with input/track-based spatialisation, it is a layer- and event-based spatialisation, integrating spatialisation at the heart of each sound synthesis, where sequences of spatialised sound particles or choruses (copies of the same event with micro-delays or spectral or spatial variations) meet a multichannel effect system.
3. It includes a library of different abstract and concrete spatialisation techniques and rendering algorithms (see [ICMC 2018 paper](https://quod.lib.umich.edu/cgi/p/pod/dod-idx/live-4-life-a-spatial-performance-tool-focused-on-rhythm.pdf?c=icmc;idno=bbp2372.2018.057;format=pdf) detailed in figure 4 of [ICMC 2021 paper](https://www.researchgate.net/publication/354526907_The_story_and_the_insides_of_a_spatial_performance_tool_Live_4_Life)), mixing channel- and object-based paradigms to be used on every sound event (to take advantage of the strengths of each approach). It has also the effect that sequences integrating channel-based approach or a multichannel system dependent are different according to the number of loudspeakers available and cannot be reproduced in the same way on different loudspeaker configurations.
