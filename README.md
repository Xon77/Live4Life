...
2. [load one](L4L_Project/_0A_Init_Live4Life.scd#L9) and [a second server](L4L_Project/_0A_Init_Live4Life.scd#L13) (:warning: **Each step of loading a server may take a few minutes depending on your computer performance. I recommend to avoid doing a lot of things with your computer during this process, since the loading of a server can be heavy.** Each server is attributed to one CPU core, so by having two severs, you can have twice as much CPU power.):
