ServerMeterView4 {
	...
	classvar serverMeterViews, updateFreq = 5, dBLow = -80, meterWidth = 9, gapWidth = -4, <height = 230;
	...
	init { arg aserver, parent, leftUp, anumIns, anumOuts;
		...
		if(numOuts > 0) {
			...
			outmeters = Array.fill( numOuts, { arg i;
				...
				.stringColor_( case {i<16} {Color.yellow}  {i.inclusivelyBetween(16,31)} {Color.red}  {i.inclusivelyBetween(32,47)} {Color.yellow} {i>48} {Color.blue} );
				levelIndic = LevelIndicator( comp, Rect(0, 0, meterWidth, 80/*180*/) ).warning_(0.9).critical_(1.0)
				.drawsPeak_(true)
				.numTicks_(9)
				.numMajorTicks_(3)
...
ServerMeter4 {
	...
	*new { |server, numIns, numOuts|
		...
		window = Window.new(server.name ++ " levels (dBFS)",
			Rect(5, 305, ServerMeterView3.getWidth(numIns, numOuts), ServerMeterView4.height),
			false);

		meterView = ServerMeterView4(server, window, 0@0, numIns, numOuts);
