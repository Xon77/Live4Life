ServerMeterView1 {
	...
	classvar serverMeterViews, updateFreq = 5, dBLow = -80, meterWidth = 20, gapWidth = 7, <height = 230;
	...
	*getWidth { arg numIns, numOuts, server;
	...
	init { arg aserver, parent, leftUp, anumIns, anumOuts;
		...
		innerView = CompositeView(view, Rect(3/*10*/, 3/*25*/, viewWidth, height) );
		...
		if(numOuts > 0) {
			...
			outmeters = Array.fill( numOuts, { arg i;
				...
				.font_(Font.sansSerif(9).boldVariant)
				.string_((i+1).asString)
				.stringColor_( case {i<8} {Color.yellow}  {i>15} {Color.yellow} {i<16} {Color.blue} );
				levelIndic = LevelIndicator( comp, Rect(0, 0, meterWidth, 80/*180*/) ).warning_(0.9).critical_(1.0)
				.drawsPeak_(true)
				.numTicks_(9)
				.numMajorTicks_(3)
...
ServerMeter1 {
	...
	*new { |server, numIns, numOuts|
		...
		window = Window.new(server.name ++ " levels (dBFS)",
			Rect(5, 305, ServerMeterView2.getWidth(numIns, numOuts), ServerMeterView2.height),
			false);
